...
class ErrorTests extends FlatSpec with Matchers {
  ...
  //     "value lol is not a member of records.Record[AnyRef{def foo: Int; def bar: Double; def baz: Double}]")
  // }

  it should "report an error on duplicate fields" in {
    typedWithMsg("""records.Record("a" -> 1, "a" -> "Hello World")""",
      "Field a is defined more than once.")
    typedWithMsg("""records.Record("a" -> 1, "a" -> "Hello World", "b" -> 3, "b" -> 3.4)""",
  ...
  it should "report an error when non-literals are used" in {
    val x = 1
    val b = ("foo", 4)
    typedWithMsg("""records.Record("a" -> x, b)""",
  ...
  it should "report an error if fields are missing" in {
    val row1 = records.Record("foo" -> 1, ("bar", 2.3), Tuple2("baz", 1.7))
    val row2 = records.Record()
  ...
  it should "report an error if fields have bad type" in {
    val row = records.Record("foo" -> "Hello", "bar" -> 5)
  ...
  it should "report an error if conversion is attempted to multi-param-list case classes" in {
    val row = records.Record("x" -> 1, "y" -> 1)
  ...
  it should "report an error if we try to access incorrectly a nested record" in {
    ...
    val row = Record("a" -> Record("b" -> Record("c" -> 1)))

    typedWithMsg("row.a.c",
      "value c is not a member of records.Record[AnyRef{def b: records.Record[AnyRef{def c: Int}]}]")
  }
  ...
  it should "report a nice error if inner fields are missing" in {
    ...
    val row = Record("a" -> Record("b" -> Record("k" -> 1, "d" -> 2)))
  ...
  it should "report a nice error if the inner type is not a case class" in {
    ...
    typedWithMsg("row.to[A]",
      "Type of field a.b: records.Record[AnyRef{def k: Int; def d: Int}] of source record doesn't conform the expected type (Int).")
  }

  // it should "report an error if a bad method is called on Rec" in {
  //   import records.Record
  //   typedWithMsg("Record.foo()",
  //     "value foo is not a member of records.Rec")
  // }

  it should "report an error if a bad method is called on Rec (named param case)" in {
    import records.Record
    typed("Record.foo(foo = 1)")
  }

  it should "report an error if Record.invokeDynamic is called with a variable method name" in {
    import records.Record
    val x = "apply"
    typedWithMsg("Record.applyDynamic(x)()",
      "You may not invoke Record.applyDynamic with a non-literal method name.")
  }
}
